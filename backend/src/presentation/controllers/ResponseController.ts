...
export const responseController = new Hono()
  .post('/create', zValidator('json', submitResponseSchema), async (c) => {
    try {
  ...
  .get('/form/:formId', zValidator('param', formIdParamSchema), async (c) => {
    try {
      const { formId } = c.req.valid('param');
  ...
  .get('/:id', zValidator('param', responseIdParamSchema), async (c) => {
    try {
  ...
  .delete('/:id', zValidator('param', responseIdParamSchema), async (c) => {
    try {
      const { id } = c.req.valid('param');
