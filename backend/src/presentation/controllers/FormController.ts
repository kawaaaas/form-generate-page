...
export const formController = new Hono()
  .post('/create', zValidator('json', createFormSchema), async (c) => {
    try {
  ...
  .get('/:id', zValidator('param', formIdParamSchema), async (c) => {
    try {
  ...
  .patch('/:id/activate', zValidator('param', formIdParamSchema), async (c) => {
    try {
      const { id } = c.req.valid('param');
